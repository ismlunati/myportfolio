                ...
                <div className="-my-6">
                    {dataAboutPage.map((data) => (
                        <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
                            <h3 className="mb-1 text-2xl font-bold sm:mb-0">{t(data.title)}</h3>
                            <div className="flex flex-col sm:flex-row items-start mb-1 
                                ...
                                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center
                                 justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0
                                  text-emerald-600 bg-emerald-100 rounded-full">{t(data.date)}</time>
                                <div className="text-xl font-bold text-gray-400">{data.subtitle}</div>
                            </div>
                            <p className="text-slate-400 max-w-md">{t(data.description)}</p>
                        </div>
                    ))}
                </div>
            </div>
